...
For a match expression shown in \Eqref{eq:complete}, the scrutinee contains a hole.By considering two rules in order, we observe that
...
Part of the definition of the internal language is shown in \Figref{fig:exptyp}. Note that the match expression takes a sequence of rules with pointer, which is different from the examples in \Secref{sec:intro}. Initially, the pointer is on the first rule (Rule \TMatchZPre). As the match expression is evaluated, we consider those rules in order, if the scrutinee doesn't match the pointed rule, the pointer would move to the next rule. Rule \TMatchNZPre gives the typing judgement for match expressions whose rule pointer has been moved away from the first rule.
...
Take \Eqref{eq:pathole-exhaustive} as an example, the pattern hole $\hehole{w}$ may be matched by any expression. Therefore, by filling the holes, the rules may cover all the possibilities of scrutinee and we shouldn't give warning on non-exhaustiveness.
...
For \Eqref{eq:pathole-exhaustive}, we notice that for all final expression $e$, either 
...
By comparing two constraints $\cpair{\cinl{\ctruth}}{\ctruth}$ and $\cpair{\cinl{\cunknown}}{\cunknown}$ that corresponds to two rules in \Eqref{eq:pathole-redundant}, we notice that
