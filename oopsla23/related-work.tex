...
We did not formalize the external language, which is bidirectionally typed, because a simply typed lambda calculus like Peanut can be turned into a bidirectional one in a semi-systematic and thus straightforward way \cite{DBLP:journals/csur/DunfieldK21}. Our implementation includes a bidirectionally typed external language, whose elaboration to Peanut, the internal language, follows \citet{DBLP:journals/pacmpl/OmarVCH19} closely.\todo{is this paragraph ok}
...
we have implemented this cast insertion machinery into Hazel.\todo{is this okay}  
This machinery is orthogonal to the machinery that we contribute in this paper, and which we intend to be relevant to systems that are not gradually typed like {GHC Haskell}, so we did not include type holes and casts here. 
...
Our contributions do not require a structure editor: they are also relevant to languages where typed holes are inserted 
...
\citet{DBLP:conf/popl/Krishnaswami09} also establishes a logical interpretation of patterns, but not pattern holes, to characterize their exhaustiveness and non-redundancy. Their algorithm operates directly on a syntax of patterns extended with logical primitives with a dualizing operation defined over them, which is similar to our inconsistency checking algorithm in character. In contrast, we express the exhaustiveness and the non-redundancy of patterns as logical entailments over a constraint language syntactically similar to patterns, and associate the entailments with a decision procedure.\todo{is this paragraph ok}
