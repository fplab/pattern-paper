  ...
  \begin{enumerate}
    \item Sufficiency:
      ...
      \item[\text{(\ref{rule:CMSPossibly})}]
        ...
        \item $\cmaysatisfy{e}{\cor{\xi}{\cfalsity}}$ \BY{Rule (\ref{rule:CPSOr1}) on \pfref{maysatisfyxi} and Lemma \ref{lem:no-e-satisfy-falsity}} \pflabel{maysatisfyxi+bot}
...
\end{corol}
\begin{proof}
  By Definition \ref{defn:nn-material-entailment} and Lemma \ref{lem:relax-nn-satisfy}.
\end{proof}
...
\begin{proof}We prove sufficiency and necessity separately.
  ...
  Let $\hexptyp{\cdot}{\Delta}{e}{\tau}$ and $\isFinal{e}$ and $\ctyp{\xi_1}{\tau}$ and $\ctyp{\xi_2}{\tau}$.
\begin{enumerate}
\item Prove sufficiency by its contraposition:
  \begin{pfsteps*}
  \item $\csatisfy{\xi_1}{\cor{\xi_2}{\cfalsity}}$ \BY{assumption} \pflabel{1=>2+bot}
  \item If $\csatisfyormay{e}{\xi_1}$ then $\csatisfy{e}{\cor{\xi_2}{\cfalsity}}$ \BY{Definition \ref{defn:const-entailment} of \pfref{1=>2+bot}} \pflabel{satisfy2+bot}
  \end{pfsteps*}

  If $\csatisfyormay{e}{\xi_1}$ then
  by rule induction over Rules (\ref{rules:Satisfy}) on conclusion of \pfref{satisfy2+bot}.
  \begin{byCases}
  \item[\text{(\ref{rule:CSOr1})}]
    \begin{pfsteps*}
  ...
  \begin{pfsteps*}
  \item $\csatisfy{\xi_1}{\xi_2}$ \BY{Definition \ref{defn:const-entailment}}
  \end{pfsteps*}

\resetpfcounter
\item Prove necessity by its contraposition:
  \begin{pfsteps*}
  \item $\csatisfy{\xi_1}{\xi_2}$ \BY{assumption} \pflabel{1=>2}
  \item If $\csatisfyormay{e}{\xi_1}$ then $\csatisfy{e}{\xi_2}$ \BY{Definition \ref{defn:const-entailment} of \pfref{1=>2}} \pflabel{satisfy2}
  \item If $\csatisfyormay{e}{\xi_1}$ then $\csatisfy{e}{\cor{\xi_2}{\cfalsity}}$ \BY{Rule (\ref{rule:CSOr1}) on the conclusion of \pfref{satisfy2}} \pflabel{defn1=>2+bot}
  \item $\csatisfy{\xi_1}{\cor{\xi_2}{\cfalsity}}$ \BY{Definition \ref{defn:const-entailment} over \pfref{defn1=>2+bot}}
  \end{pfsteps*}
\end{enumerate}
\end{proof}
