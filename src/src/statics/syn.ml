...
let rec syn_exp (gamma : VarCtx.t) (delta : HoleCtx.t) : Exp.t -> Typ.t =
  ...
  | Ap (f, exp) -> (
      ...
      | Arrow (typ1, typ2) ->
          ...
          if typ1 == typ1' then typ2 else raise (Error (Our TypeIncon))
      | _ -> raise (Error (Our FunNotArrow)))
  | Pair (e1, e2) ->
      let typ1, typ2 = (syn_exp gamma delta e1, syn_exp gamma delta e2) in
      Prod (typ1, typ2)
  | Inj (side, typ2, e) -> (
  ...
  | Match (scrut, zrules) -> (
      ...
      | ZRules ([], r, rs) ->
          ...
          let _, xi, typ' =
            syn_rules gamma delta (r :: rs) typ Constraint.Falsity
          in
          if Incon.is_exhaustive xi then typ'
          else raise (Error (Their NonExhaustive))
      | ZRules (rs_pre, r, rs_post) ->
          let typ = syn_exp gamma delta scrut in
          if not (Eval.is_final scrut) then raise (Error (Our ScrutNotFinal))
          else
            let count_pre, xi_pre, typ' =
              syn_rules gamma delta rs_pre typ Constraint.Falsity
            in
            (* TODO check if e mustn't satisfy xi_pre *)
            let _, xi_rest, typ'' =
              syn_rules gamma delta ~index:count_pre (r :: rs_post) typ xi_pre
            in
            if typ' <> typ'' then raise (Error (Our (BranchIncon count_pre)))
            else if not (Incon.is_exhaustive (Constraint.Or (xi_pre, xi_rest)))
            then raise (Error (Their NonExhaustive))
            else typ')

and syn_rules gamma delta ?(index = 1) rules typ xi_pre :
    int * Constraint.t * Typ.t =
  match rules |> List.map (fun rule -> syn_rule gamma delta rule typ) with
      ...
      if Incon.is_redundant xi_r xi_pre then
        ...
        List.fold_left
          (fun (i, xi_pre, typ'_last) (xi_r, typ') ->
            ...
            else (i + 1, Constraint.Or (xi_pre, xi_r), typ'))
  ...
  let typ' =
    try
      syn_exp
        ...
        exp
    with
    | VarCtx.DuplicateVarInPat x -> raise (Error (Our (DupVar x)))
    | HoleCtx.DuplicateHoleName h -> raise (Error (Our (DupHole h)))
  in
  (xi, typ')
...
and ana_pat (pat : Pat.t) (typ : Typ.t) : Constraint.t * VarCtx.t * HoleCtx.t =
  match (pat, typ) with
  | EmptyHole h, typ ->
      (Unknown, VarCtx.empty, HoleCtx.add h (HoleCtx.PatHole, typ) HoleCtx.empty)
  ...
  | Pair (p1, p2), Prod (typ1, typ2) -> (
      let (xi1, gamma1, delta1), (xi2, gamma2, delta2) =
...
let syn gamma delta exp : unit =
  try
    let _ = syn_exp gamma delta exp in
    ()
  with
  | Error (Our fault) -> (
      match fault with
      ...
      | DupVar x ->
          Printf.printf "Duplicate variable %s in a pattern\n" (Var.to_string x)
      | FunNotArrow ->
          Printf.printf "A non-arrow-type expression is used like a function\n"
      | TypeIncon -> Printf.printf "Type inconsistency occurs\n"
      | BranchIncon i -> Printf.printf "Branch %d breaks the consistency\n" i
      | EmptyRules ->
          Printf.printf "Match expression should at least have one rule\n"
      | TypeAnnIncon ->
          Printf.printf "The annotated type is inconsistent with the pattern\n"
      | ScrutNotFinal ->
          Printf.printf
            "The scrutinee of intermediate match expressions must be final\n")
  | Error (Their fault) -> (
      match fault with
      | Redundant i -> Printf.printf "Redundant rule %d\n" i
      | NonExhaustive ->
          Printf.printf "The match expression mustn't be exhaustive\n")
       ...
       ( Pair (Inj (L, Num, EmptyHole 1), Num 2),
         ZRules
           ( [],
             Rule (Pair (Inj (R, Num 0), Var "x"), Var "x"),
             [
               Rule (Pair (Inj (R, Num 1), Var "x"), Var "x"); Rule (Wild, Num 3);
             ] ) ));
  [%expect {| |}]
  ...
  syn
    ...
    (Match
       ( Pair (Inj (L, Num, EmptyHole 1), Num 2),
         ZRules
           ( [],
             Rule (Pair (Inj (R, Num 2), Var "x"), Var "x"),
             (* (inr(2), x) *)
             [
               Rule (Pair (Inj (R, Var "x"), Num 3), Var "x");
               (* (inr(x), 3) *)
               Rule (Pair (Inj (L, Var "x"), Num 2), Var "x");
               Rule (Pair (Inj (R, Num 2), Num 3), Var "x");
               (* (inr(2), 3) *)
             ] ) ));
  syn VarCtx.empty
    (HoleCtx.singleton 1 (HoleCtx.ExpHole, Typ.Num))
         ...
         ZRules
           ( [],
             ...
             [
               ...
               Rule (Pair (Inj (R, EmptyHole 2), EmptyHole 3), Var "x");
               (* (inr(hole), hole) *)
             ] ) ));
  syn VarCtx.empty
    (HoleCtx.singleton 1 (HoleCtx.ExpHole, Typ.Num))
           ...
           ( [],
             ...
             [
               Rule
                 ( Pair
                     ( Inj (R, Var "x"),
                       NonEmptyHole (3, Prod (Num, Num), Pair (Num 1, Num 2)) ),
                   Var "x" );
...
let%expect_test "[pat-hole]not-exhaustive" =
  syn
    ...
    (Match
       ( Pair (Num 1, Num 2),
         ZRules ([], Rule (Pair (Num 2, EmptyHole 3), Var "x"), []) ));
  syn
    (VarCtx.singleton "x" Typ.Num)
    (HoleCtx.singleton 1 (HoleCtx.ExpHole, Typ.Num))
    (Match
       ( Pair (Inj (L, Num, EmptyHole 1), Num 2),
         ZRules
           ( [],
             Rule (Pair (Inj (R, EmptyHole 2), Var "x"), Var "x"),
             (* (inr(2), x) *)
             [
               Rule (Pair (Inj (R, Var "x"), EmptyHole 3), Var "x");
               (* (inr(x), 3) *)
         ...
         ZRules
           ( [],
             ...
             [
               ...
               (* If this and rule below are flipped, redundant*)
               Rule (Pair (Inj (L, Num 2), Var "x"), Var "x");
             ] ) ));
  [%expect
    {|
         ...
         ZRules
           ( [],
             ...
             [
               ...
               (* (inr(x), 3) *)
               Rule (Pair (Inj (L, Var "x"), Num 2), Var "x");
               Rule (Pair (Inj (R, EmptyHole 2), Num 3), Var "x");
               (* (inr(hole), 3) *)
             ] ) ));
  syn VarCtx.empty
    (HoleCtx.singleton 1 (HoleCtx.ExpHole, Typ.Num))
         ...
         ZRules
           ( [],
             ...
             [
               ...
               (* (inr(x), 3) *)
               Rule (Pair (Inj (L, Num 2), Var "x"), Var "x");
               (* If this and rule below are flipped, not exhaustive*)
               Rule (Pair (Inj (L, Num 2), EmptyHole 4), Var "x");
             ] ) ));
  [%expect {|
    Redundant rule 4
