...
Next, in Sec.~\ref{sec:constraint}-\ref{sec:algorithm}, we define a corresponding static semantics as a type assignment system together with a match constraint language that we use to reason about exhaustiveness and redundancy in the presence of
holes. We discuss the Agda mechanization of the metatheory in Sec.~\ref{sec:agda}. Finally, in \autoref{sec:decidability}, we consider algorithmic issues.
...
First, the judgment $\hpatmatch{e}{p}{\theta}$ denotes a successful match as witnessed by the substitution $\theta$ consisting of the variables bound in $p$. Second, the judgment $\hnotmatch{e}{p}$ denotes that $e$ does not match $p$. Third, the judgment $\hmaymatch{e}{p}$ indicates an \emph{indeterminate match} due to the presence of holes in $e$ or $p$. Note that all three judgments appear in the the premises of Rules \ITSuccMatch, \ITFailMatch, and \IMatch respectively. 
...
In \autoref{sec:statics}, we will see how match constraint language enables static reasoning of exhaustiveness and redundancy. The judgment $\chpattyp{p}{\tau}{\xi}{\Gamma}{\Delta}$ in \autoref{fig:pat-rulestyp}, detailed in the next section, generates constraints from patterns.
...
To simply the proof of progress (part of \autoref{theorem:determinism}), we consider only final expressions in the following definition of entailment. 
...
single rule $r$, and series of rules $rs$. We will describe the emission of constraint from and its use in check redundancy of a rule $r$ with respect
