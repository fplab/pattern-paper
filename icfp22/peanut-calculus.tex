...
to predict the runtime behavior of match expressions by introducing a static semantics that checks exhaustiveness and redundancy. In order to perform such checking of match expressions in \autoref{fig:exhaustiveness},
\ref{fig:redundancy}, we must statically predict the result of pattern matching. To do so, we start by introducing a \textit{match constraint language}, which
...
We begin with the entailment judgement associated with redundancy checking. Recall in Rules \TOneRules and \TRules, we use $\cnotsatisfy{\xi_r}{\xi_{pre}}$ to ensure the branch $r$ of our interest does not have to be redundant with respect to its preceding branches $rs_{pre}$. 
...
Recall in Rules \TMatchZPre and \TMatchNZPre, we use $\csatisfyormay{\ctruth}{\xi}$ to ensure that the constituent rules in a match expression either determinately or indeterminately exhaustive. 
