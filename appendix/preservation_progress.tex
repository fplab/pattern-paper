...
\item Sufficiency:
  ...
  As a result,
    \begin{pfsteps*}
    \item for all $e$ such that $\hexptyp{\cdot}{\Delta}{e}{\tau}$ and $\isFinal{e}$ \\
        we have $\csatisfyormay{e}{\xi_1}$ implies $\csatisfy{e}{\xi_2}$ \pflabel{defn:1|=2}
    \item $\csatisfy{\xi_1}{\xi_2}$ \BY{Definition \ref{defn:const-entailment} on \pfref{defn:1|=2}} \pflabel{1satisfy2}
    \end{pfsteps*}
...
\item Necessity:
  ...
  \begin{pfsteps*}
  \item for all $e$ such that $\hexptyp{\cdot}{\Delta}{e}{\tau}$ and $\isFinal{e}$ \\
    we have $\csatisfyormay{e}{\xi_1}$ implies $\csatisfy{e}{\cor{\xi_2}{\cfalsity}}$ \pflabel{defn:1|=2vbot}
  \item $\csatisfy{\xi_1}{\cor{\xi_2}{\cfalsity}}$ \BY{Definition \ref{defn:const-entailment} on \pfref{defn:1|=2vbot}} \pflabel{1|=2vbot}
  \end{pfsteps*}
...
\begin{proof}
  ...
  \item[\text{(\ref{rule:CTZeroRule})}]
    ...
    \item $\cnotsatisfy{\xi_r}{\xi_{pre}}$ \BY{Lemma \ref{lem:relax-not-redundant} on \pfref{r|/=pre+rs}} \pflabel{r|/=pre}
    \item $\chrulstyp{\Gamma}{\Delta}{\cor{\xi_{pre}}{\xi_r}}{\cdot}{\tau}{\cfalsity}{\tau'}$ \BY{Rule (\ref{rule:CTZeroRule})} \pflabel{zeroType}
    \item $\chrulstyp{\Gamma}{\Delta}{\xi_{pre}}{\hrulesP{r}{\cdot}}{\tau}{\cor{\xi_r}{\cfalsity}}{\tau'}$ \BY{Rule (\ref{rule:CTRules}) on \pfref{rType} and \pfref{zeroType} and \pfref{r|/=pre}} \pflabel{[zero]conc}
    \item $\chrulstyp{\Gamma}{\Delta}{\xi_{pre}}{\rmpointer{\zruls{\cdot}{r}{\cdot}}}{\tau}{\cor{\xi_r}{\cfalsity}}{\tau'}$\todolater{commut} \BY{Definition \ref{defn:rmpointer} on \pfref{[zero]conc}}
    \end{pfsteps*}
  ...
  \item[\text{(\ref{rule:CTRules})}]
    ...
    \item $\chrulstyp{\Gamma}{\Delta}{\cor{\xi_{pre}}{\xi_r'}}{\rmpointer{\zruls{rs'}{r}{\cdot}}}{\tau}{\cor{\xi_{rs}'}{\xi_r}}{\tau'}$\todolater{assoc} \BY{IH on \pfref{rs'Type} and \pfref{rType} and \pfref{r|/=pre+rs}} \pflabel{rs'+rType}
    ...
    \item $\chrulstyp{\Gamma}{\Delta}{\xi_{pre}}{\rmpointer{\zruls{\hrulesP{r'}{rs'}}{r}{\cdot}}}{\tau}{\cor{\xi_r'}{\cor{\xi_{rs}'}{\xi_r}}}{\tau'}$ \BY{Definition \ref{defn:rmpointer} on \pfref{[zero]conc}}
    \end{pfsteps*}
  \resetpfcounter
  \end{byCases}
\end{proof}
...
\item[\text{(\ref{rule:TMatchZPre})}]
  ...
  \item[\text{(\ref{rule:ITExpMatch})}]
    ...
    \item $\hexptyp{\cdot}{\Delta}{\hmatch{e_1'}{\zruls{\cdot}{\hrulP{p_r}{e_r}}{rs}}}{\tau}$ \BY{Rule (\ref{rule:TMatchZPre}) on \pfref{newscrutType} and \pfref{rulesType} and \pfref{exhaust}}
    \end{pfsteps*}
  ...
  \item $\cnotsatisfyormay{e_1}{\cor{\xi_r}{\cfalsity}}$ \BY{Lemma (\ref{lem:relax-nn-satisfy}) on \pfref{notrConst}} \pflabel{notpre+rConst}
    ...
    \item $\hexptyp{\cdot}{\Delta}{\hmatch{e_1}{\zruls{\hrulesP{\hrul{p_r}{e_r}}{\cdot}}{r'}{rs'}}}{\tau}$ \BY{Rule (\ref{rule:TMatchNZPre}) on \pfref{scrutType} and \pfref{scrutFinal} and \pfref{pre+rType} and \pfref{postrulesType} and \pfref{notpre+rConst} and \pfref{newexhaust}}
    \end{pfsteps*}
  \end{byCases}
...
\item[\text{(\ref{rule:TMatchNZPre})}]
  ...
  \item[\text{(\ref{rule:ITFailMatch})}]
    ...
    \item $\hexptyp{\cdot}{\Delta}{\hmatch{e_1}{\zruls{\rmpointer{\zruls{rs_{pre}}{\hrul{p_r}{e_r}}{\cdot}}}{r'}{rs'}}}{\tau}$ \BY{Rule (\ref{rule:TMatchNZPre}) on \pfref{scrutType} and \pfref{scrutFinal} and \pfref{pre+rrulesType} and \pfref{postrulesType} and \pfref{notsatisfypre+r} and \pfref{exhaust}} 
    \end{pfsteps*}
  \end{byCases}
\resetpfcounter
\end{byCases}
