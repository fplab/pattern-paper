...
\begin{proof}
  ...
  \item[\text{(\ref{rule:CMSNotIntro})}]
    ...
    \end{pfsteps*}
    By \autoref{lem:notintro-refutable} on \pfref{notintro} and \pfref{rft} and case analysis on its conclusion.
    ...
    \item[\refutable{\ctruify{\xi}}]
      ...
      \item $\cmaysatisfy{e}{\ctruify{\xi}}$ \BY{\ruleref{rule:CMSNotIntro} on \pfref{notintro} and \pfref{rft-truify}} \pflabel{[notintro]maysat}
      \item $\csatisfyormay{e}{\ctruify{\xi}}$ \BY{Rule (\ref{rule:CSMSSat}) on \pfref{[notintro]maysat}}
      \end{pfsteps*}
    \restorelocalsteps{lem:maysat-satormay-truify-2}
    \item[\csatisfy{e}{\ctruify{\xi}}]
      \begin{pfsteps*}
      \item $\csatisfy{e}{\ctruify{\xi}}$ \BY{assumption} \pflabel{[notintro]satisfy}
      \item $\csatisfyormay{e}{\ctruth}$ \BY{Rule (\ref{rule:CSMSSat}) on \pfref{[notintro]satisfy}}
      \end{pfsteps*}
    \end{byCases}
    ...
    \item Sufficiency:
      ...
      \item[\text{(\ref{rule:CSMSMay})}]
        ...
        \item[\text{(\ref{rule:CMSNotIntro})}] 
          ...
          \item $\notIntro{e}$ \BY{assumption} \pflabel{notintro}
          \end{pfsteps*}
          By rule induction over \rulesref{rules:notintro} on \pfref{notintro}, for each case,
